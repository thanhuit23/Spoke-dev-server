...
// 폰트 로드
function fontLoader(fontname, fontUrl) {
...
// 텍스트 텍스처 생성
export async function createTextTexture(textProperty) {
    // 기본 속성
    const property = {
        text : "", // 텍스트
    ...
    // 텍스트 라인 높이
    //property.lineHeight = property.fontSize;
    ...
    property.textHeight += (property.paddingTop + property.paddingBottom);
    ...
    context.font = property.fontWeight + " " + property.fontSize + "px " + property.fontName;
    context.textAlign = property.textAlign;
    ...
    // 텍스트 배열을 정렬하여 출력
    for (let index = 0; index < texts.length; index++) {
        let textPositionY = (property.lineHeight * 0.5) + ((property.lineHeight + property.lineSpace) * index) + 1;
        if (index == 0 && property.paddingTop > 0) {
            textPositionY += property.paddingTop;
        }
        context.fillText(texts[index], textPositionX, textPositionY); // lineHeight
    }
...
// 텍스트 머티리얼 생성
export async function createTextMaterial(textProperty) {
...
// 텍스트 매쉬 생성
export async function createTextMesh(textProperty) {
